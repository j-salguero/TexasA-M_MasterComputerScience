...
#define MAX_LIST_SIZE   300000000 // Max = 2^28, so made this slightly larger
...
//
pthread_t threads[MAX_THREADS];         // Store thread IDs
pthread_barrier_t barrier;              // Barrier for keeping threads in sync
typedef struct {           // Thread-specific arguments
    int thread_id;      // Thread's ID
    int q;          
} thread_arg_t;
// Since the threads will all be getting their own unique section of the list without overlapping sections, don't need to initialize a mutex lock
...
int ptr[MAX_THREADS + 1];   //Added global variable for threads
...
//
// Sort list using parallelized threads
void *sort_list_thread(void* arg) {

    // Need to extract information from the thread-specific arguments
    thread_arg_t *thread_arg = (thread_arg_t *)arg;
    int my_id = thread_arg->thread_id;
    int q = thread_arg->q;

    // Determine which portion each thread will be working on
    int np = list_size/num_threads; 	// Sub list size 
    int my_list_size = ptr[my_id + 1] - ptr[my_id];     // Each subset should be the same size = np since work was split evenly, but calculated my_list_size to be safe

    // Sort each thread's sublist
    qsort(&list[ptr[my_id]], my_list_size, sizeof(int), compare_int);        
    pthread_barrier_wait(&barrier);     // Use barrier to wait for all threads to be finished sorting before moving on
...
if (DEBUG) print_list(list, list_size); 
    ...
    // Merge together the sorted sublists from various threads
    for(int level = 0; level < q; level++) {
		...
		pthread_barrier_wait(&barrier);         // Wait for all threads to finish sorting before moving on
		...
		pthread_barrier_wait(&barrier);         // Wait for all threads to finish sorting their sublist before moving on
...
// Original serial Merge function - rewrote this function for thread integration
...
// VS: ... parallel merge sort
    ...
    //    sort_list_thread(q);       //sort_list(q);
    ...
    for (int t = 0; t < num_threads; t++) {
        args[t].thread_id = t;
        args[t].q = q;
        pthread_create(&threads[t], NULL, sort_list_thread, &args[t]);  // Call sort function here
    }
