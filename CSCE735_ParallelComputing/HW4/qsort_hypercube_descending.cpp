...
int * HyperCube_Class::merged_list(int * list1, int list1_size, int * list2, int list2_size) {
	...
	if (list1[idx1] >= list2[idx2]) { // Flip comparison for descending order
	    list[idx] = list1[idx1]; 
	    idx++; idx1++;
	} else {
	    list[idx] = list2[idx2]; 
...
int HyperCube_Class::split_list_index (int *list, int list_size, int pivot) {
    ...
    while (first < last) {
	if (list[mid] >= pivot) { // Flip comparison for descending order
...
void HyperCube_Class::check_list() {
	...
	for (j = 1; j < list_size; j++) {
	    if (list[j] > list[j-1]) local_error = 1; // Flip comparison for descending order
	}
...
void HyperCube_Class::HyperCube_QuickSort() {
	...
	// ***** Add MPI call here *****
	MPI_Allreduce(&local_median, &pivot, 1, MPI_INT, MPI_SUM, sub_hypercube_comm); // compute the sum of local_median values of hypercube
	...
	if (nbr_k < my_id) { // Flip comparison for descending order
	    ...
	    // ***** Add MPI call here *****
		MPI_Send(list, list_size_leq, MPI_INT, nbr_k, tag, MPI_COMM_WORLD);
...
int compare_int(const void *a0, const void *b0) {
    int a = *(int *)a0;
    int b = *(int *)b0;
    if (a > b) { // Flip comparison for descending order
	return -1; 
    } else if (a < b) { // Flip comparison for descending order
	return 1;
    } else {
	return 0;
    }
